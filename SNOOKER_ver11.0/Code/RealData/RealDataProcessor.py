...
def calculateDiffTimeFromTimestamp(time_1, time_2):
    ...
    return round(time_diff.total_seconds() / 60, 1)
    #return time_diff
...
def cleanDataset(data):
    ...
    data = data[~data['description'].str.contains("PAM")]

    data["time_stamp_converted"] = data["time_stamp"].apply(cleanTimestamps)
    #print("Type", data["time_stamp_converted"])
    ...
    data['time_stamp'] = data['time_stamp'].apply(lambda x: x[1:-1].split(', '))
    
    with_negatives = len(data)
    data = data[data["time_stamp"].apply(findNegativeTimestamps) != False]
...
def getOperators(ticket_id, subfamily, transitions, summary, timestamps, operators_info):
    ...
    curr_user = ""
    user_timestamp = -1
    #print("Transição", transitions)
    #print("Descricao", summary)
...
def analyseDataset(data):
    ...
    #print(mean)
    transitions_info.to_csv("../RealDatasetCleaner/Resilient/transitions_info.csv", sep=";", index = False)
...
df = pd.read_csv('../RealDatasetCleaner/Resilient/realDataset_updated.csv', sep=";", index_col=0)
keep_col = ['ref_num', 'discovered_date', 'end_date', 'org_name', 'category', 'alert_code', 'incident_status', 'action_desc (description without comments from operators)', 'type', 'time_stamp'] 

df = df[keep_col]
df = df.rename(columns={'ref_num': 'ID', 
                   #'discovered_date': 'Found (UTC)',
                   #'end_date': 'Fixed', 
                   'org_name': 'Client',
                   'category': 'Family',
                   'alert_code': 'Subfamily',
                   'incident_status': 'Status',
                   'action_desc (description without comments from operators)': 'description',
                   'type': 'Transitions'
                   })
...
df.to_csv("../RealDatasetCleaner/Resilient/realDataset_updated_cleaned.csv", sep=";", index = False)
